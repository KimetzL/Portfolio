              ...
              {projects.filter(p => p.featured).map((project, index) => (
                ...
                >
                  <Card className="h-full overflow-hidden hover:shadow-xl transition-all duration-300 border-primary/30 bg-card/80 backdrop-blur">
                    <div className={`aspect-video relative overflow-hidden group ${project.image.includes('chatbot') ? 'bg-[#e0f7fc] dark:bg-[#0c2832]' : 'bg-muted'}`}>
                      <img
                        src={project.image}
                        alt={project.title}
                        className={`w-full h-full group-hover:scale-105 transition-transform duration-500 ${project.image.includes('chatbot') ? 'object-contain p-2' : 'object-cover object-center'}`}
                      />
                    </div>
                  ...
                  <Card className="h-full overflow-hidden hover:shadow-lg transition-shadow">
                    <div className={`aspect-video relative overflow-hidden group ${project.image.includes('chatbot') ? 'bg-[#e0f7fc] dark:bg-[#0c2832]' : 'bg-muted'}`}>
                      <img
                        src={project.image}
                        alt={project.title}
                        className={`w-full h-full group-hover:scale-105 transition-transform duration-300 ${project.image.includes('chatbot') ? 'object-contain p-2' : 'object-cover object-center'}`}
                      />
                    </div>
                    <CardHeader>
